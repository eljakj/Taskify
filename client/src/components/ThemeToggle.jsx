export default function ThemeToggle({ theme, toggleTheme }) {
  ...
  return (
    <button
      type="button"
      onClick={toggleTheme}
      className="group relative flex h-14 w-[124px] cursor-pointer items-center rounded-full border border-zinc-200 bg-white/85 px-2 shadow-sm transition hover:-translate-y-0.5 dark:border-slate-700 dark:bg-slate-900/85"
      aria-label="Toggle theme"
      title={isDark ? "Switch to light mode" : "Switch to dark mode"}
    >
      <span
        className={`absolute top-1.5 h-11 w-11 rounded-full bg-indigo-600 shadow-lg shadow-indigo-600/25 transition-all duration-300 ${
          isDark ? "left-[70px]" : "left-1.5"
        }`}
      />
